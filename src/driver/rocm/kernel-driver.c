...
int tnn_integrate_network(struct rate_library* rates, struct tnn* network,
    ...
    if (options.rocm_accel) {
        ...
        devbuf_create(args[0], rates->p0,
                      rates->number_reactions * sizeof(real_t));
        ...
        devbuf_create(args[11], params->f_plus,
                      params->f_plus_total * sizeof(real_t));
        devbuf_create(args[12], params->f_minus,
                      params->f_minus_total * sizeof(real_t));
        ...
        devbuf_create(args[15], params->f_plus_sum,
                      network->info->number_species * sizeof(real_t));
        ...
        real_t* real_t_val = malloc(sizeof(real_t) * 3);
        real_t_val[0] = network->f->t9;
        real_t_val[1] = network->f->t_max;
        real_t_val[2] = network->f->dt_init;
        devbuf_create(args[27], real_t_val, 3 * sizeof(real_t));
        free(real_t_val);
